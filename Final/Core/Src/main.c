...
int main(void)
{
	...
	while (1)
	{
		...
		if (mode == 2 && last_stand == 0) //BattleMode
		{
			...
			sprintf(sensorData1, "Temperature: %f (C)\r\n Pressure: %f\r\n Humidity: %f",  temp_data, pressure_data, humidity_data);
			HAL_UART_Transmit(&huart1, (uint8_t*)sensorData1, strlen(sensorData1), 0xFFFF);
			...
			char sensorData2[100];
			sprintf(sensorData2, "Accelero X: %f, Y: %f, Z: %f (m/s^2)\r\n", accel_data[0], accel_data[1], accel_data[2]);
			HAL_UART_Transmit(&huart1, (uint8_t*)sensorData2, strlen(sensorData2), 0xFFFF);
